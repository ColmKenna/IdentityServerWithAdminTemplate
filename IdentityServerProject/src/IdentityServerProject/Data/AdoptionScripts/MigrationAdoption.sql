...
DECLARE @ContextName nvarchar(128) = N'$(ContextName)';
DECLARE @BaselineMigrationId nvarchar(150) = N'$(BaselineMigrationId)';
DECLARE @ProductVersion nvarchar(32) = N'$(ProductVersion)';
DECLARE @ExpectedFingerprint varchar(64) = '$(ExpectedFingerprint)';
DECLARE @ExpectedItemCount bigint = $(ExpectedItemCount);

BEGIN TRY
    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
    BEGIN TRANSACTION;

    DECLARE @HistoryExisted bit = CASE
    ...
    BEGIN
        COMMIT TRANSACTION;
        PRINT CONCAT(@ContextName, N' baseline is already adopted.');
        RETURN;
    END;
    ...
    BEGIN
        DECLARE @UnexpectedHistoryMessage nvarchar(2048) = CONCAT(
            @ContextName,
            N' has migration history but is missing the expected baseline ',
            @BaselineMigrationId,
            N'. Refusing to stamp it.');
        THROW 51000, @UnexpectedHistoryMessage, 1;
    END;
    ...
    BEGIN
        DECLARE @EmptyMessage nvarchar(2048) = CONCAT(
            @ContextName,
            N' is empty. Run the normal migration bundle instead of adopting it.');
        THROW 51001, @EmptyMessage, 1;
    END;

    DECLARE @ActualFingerprint varchar(64);
    DECLARE @ActualItemCount bigint;
    ...
    SELECT
        @ActualFingerprint = CONVERT(varchar(64), HASHBYTES(
            'SHA2_256',
            STRING_AGG(CONVERT(nvarchar(max), Item) COLLATE DATABASE_DEFAULT, NCHAR(10))
                WITHIN GROUP (ORDER BY Item COLLATE DATABASE_DEFAULT)), 2),
        @ActualItemCount = COUNT_BIG(*)
    FROM SchemaItems;

    IF @ActualFingerprint <> @ExpectedFingerprint OR @ActualItemCount <> @ExpectedItemCount
    BEGIN
        DECLARE @MismatchMessage nvarchar(2048) = CONCAT(
            @ContextName,
    ...
    COMMIT TRANSACTION;
    PRINT CONCAT(@ContextName, N' baseline adoption completed.');
END TRY
BEGIN CATCH
    IF XACT_STATE() <> 0
    BEGIN
