...
public interface IIdentityResourceListService
{
    ...
    /// <summary>
    ///     Deletes the named Identity Resource, refusing the operation if any client still references it or if it is
    ///     non-editable.
    /// </summary>
    Task<IdentityResourceDeleteResult> DeleteIdentityResourceAsync(string name,
        CancellationToken cancellationToken = default);
}