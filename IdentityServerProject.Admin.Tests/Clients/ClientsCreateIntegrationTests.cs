...
public class ClientsCreateIntegrationTests : IDisposable
{
    ...
    {
        HttpResponseMessage getResponse = await _client.GetAsync("/Admin/Clients/Create");
    ...
    public async Task OnGet_Should_Return200OK_AndRenderCreateFormWithPresetCards()
    {
        ...
        IElement? title = doc.QuerySelector(".page-title");
        Assert.NotNull(title);
        Assert.Contains("Register New Client", title.TextContent);

        // Verify preset cards exist
        IHtmlCollection<IElement> presetCards = doc.QuerySelectorAll(".preset-card");
        Assert.Equal(4, presetCards.Length);

        // Verify required form elements
        IElement? clientIdInput = doc.QuerySelector("input[name='Input.ClientId']");
        IElement? clientNameInput = doc.QuerySelector("input[name='Input.ClientName']");
    ...
    public async Task OnPost_Should_CreateClient_AndDisplaySecretRevealBanner()
    {
        ...
        string clientId = $"web-app-{Guid.NewGuid():N}";
        ...
        Assert.DoesNotContain("token", postResponse.Headers.Location?.OriginalString,
        ...
        string responseCookies = postResponse.Headers.Contains("Set-Cookie")
        ...
        IElement? secretBanner = doc.QuerySelector("#secret-reveal-banner");
        ...
        IDocument refreshDocument = await GetDocumentAsync(refreshResponse);
    ...
    public async Task OnPost_Should_ShowValidationError_When_ClientIdIsMissing()
    {
        ...
        IDocument doc = await GetDocumentAsync(response);
        IElement? validationSummary = doc.QuerySelector(".validation-summary");
    ...
    {
        (string token, string cookie) = await GetAntiforgeryTokenAndCookieAsync();
        string clientId = $"overlong-http-{Guid.NewGuid():N}";
        var formData = new Dictionary<string, string>
        {
            { "__RequestVerificationToken", token },
            { "Input.ClientId", clientId },
