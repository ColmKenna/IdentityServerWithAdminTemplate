...
public static class SeedData
{
        ...
        string sysAdminPassword)
    {
        ...
        {
            var roleResult = await roleManager.CreateAsync(new IdentityRole(Config.SysAdminRole));
        ...
        await EnsureUserAsync(userManager, sysAdminEmail, "System Administrator", sysAdminPassword, Config.SysAdminRole);
    }
        ...
        string testUserPassword)
    {
        await SeedSysAdminAsync(
            ...
            sysAdminEmail,
            sysAdminPassword);
        await EnsureUserAsync(userManager, "testuser@sales.local", "Test User", testUserPassword, role: null);
    }
    ...
    private static async Task SeedResourcesAsync(ConfigurationDbContext configDb)
    {
        ...
        var existingIdentityResources = await configDb.IdentityResources.ToListAsync();
        foreach (var resource in Config.IdentityResources)
        {
            var existing = existingIdentityResources.FirstOrDefault(r => r.Name == resource.Name);
        ...
        var existingApiScopes = await configDb.ApiScopes.Select(s => s.Name).ToListAsync();
        ...
        var existingApiResources = await configDb.ApiResources.Select(r => r.Name).ToListAsync();
