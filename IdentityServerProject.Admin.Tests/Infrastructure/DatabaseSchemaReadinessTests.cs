...
public sealed class DatabaseSchemaReadinessTests
{
    ...
    public async Task FullyMigratedIdentityDatabase_ContainsAuditFilterIndexes()
    {
        ...
        while (await reader.ReadAsync()) actualIndexes.Add(reader.GetString(0));

        foreach (string expectedIndex in expectedIndexes) Assert.Contains(expectedIndex, actualIndexes);
    }

    [Fact]
    public async Task LegacySchemaWithoutHistory_FailsBeforeSeedAndNamesRequiredBundle()
    {
        string databaseName = $"Task06_Pending_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            await _factory.RunInScopeAsync(async services =>
            {
                ...
                InvalidOperationException error =
                    await Assert.ThrowsAsync<InvalidOperationException>(() => validator.EnsureReadyAsync());
