...
public class ClientsClonePageModelTests
{
        ...
        Mock<ISecretRevealService>? revealService = null)
    {
        ...
        return new CloneModel(createService.Object, detailsService.Object,
            (revealService ?? new Mock<ISecretRevealService>()).Object)
        {
    ...
    public async Task OnPostAsync_SourceNotFound_ReturnsNotFoundWithoutCloning()
    {
        ...
        createService.Verify(
            s => s.CloneClientAsync(It.IsAny<string>(), It.IsAny<ClientCreateInputModel>(),
    ...
    public async Task OnPostAsync_ValidationFailure_MapsUnprefixedFieldErrorAndRedisplaysPage()
    {
        ...
        Assert.IsType<PageResult>(result);
        Assert.Equal("Source Client", model.SourceClientName);
        Assert.Equal("A client with this ID already exists.",
            model.ModelState["Input.ClientId"]!.Errors.Single().ErrorMessage);
    }
    ...
    public async Task OnPostAsync_SuccessWithSecret_StoresOnlyRevealHandleAndRedirectsToCreate()
    {
        ...
        revealService.Setup(s =>
                s.IssueAsync(new SecretRevealTarget(SecretRevealPurpose.ClientCreated, "target-client"),
                    "plaintext-secret", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SecretRevealTicket(SecretRevealHandle.Create("opaque-handle"),
                DateTimeOffset.UtcNow.AddMinutes(5)));
        CloneModel model = CreateModel(createService, detailsService, revealService);
        model.SourceClientId = "source-client";
        model.Input = new ClientCreateInputModel { ClientId = "target-client", ClientName = "Target Client" };
