...
public interface IApiResourceEditorService
{
    ...
    Task<AdminMutationResult> RevokeSecretAsync(ScopeName name, int secretId,
        CancellationToken cancellationToken = default);

    /// <summary>Attaches an existing system-wide API scope to the resource.</summary>
    Task<AdminMutationResult> AttachScopeAsync(ScopeName name, ScopeName scopeName,
        CancellationToken cancellationToken = default);
    ...
    Task<AdminMutationResult> DetachScopeAsync(ScopeName name, ScopeName scopeName,
        CancellationToken cancellationToken = default);

    /// <summary>Adds a user claim type to the resource.</summary>
    Task<AdminMutationResult> AddClaimAsync(AddApiResourceClaimCommand command,
        CancellationToken cancellationToken = default);

    /// <summary>Removes a user claim type from the resource.</summary>
    Task<AdminMutationResult> RemoveClaimAsync(ScopeName name, ClaimType claimType,
        CancellationToken cancellationToken = default);

    /// <summary>Enables or disables the resource.</summary>
    Task<AdminMutationResult> SetEnabledAsync(ScopeName name, bool enabled,
        CancellationToken cancellationToken = default);
