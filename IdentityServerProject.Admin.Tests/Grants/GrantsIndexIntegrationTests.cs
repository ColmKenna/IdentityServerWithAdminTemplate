...
public class GrantsIndexIntegrationTests : IDisposable
{
    ...
    private HttpClient CreateClient(IGrantListService grantListService)
    {
        ...
        {
            builder.ConfigureTestServices(services => { services.AddSingleton(grantListService); });
        });
    ...
    private static IGrantListService MockService(ListResult<GrantListItem> result)
    {
        ...
        mock.Setup(s =>
                s.GetGrantsAsync(It.IsAny<GrantFilter?>(), It.IsAny<Pagination>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(result);
        return mock.Object;
    }
    ...
    {
        GrantListItem item1 = MakeItem("grant-key-1", "client-1", "Client One", "user-101");
        GrantListItem item2 = MakeItem("grant-key-2", "client-2", "Client Two", "user-102");

        IGrantListService service = MockService(new ListResult<GrantListItem>
        {
        ...
        HttpResponseMessage response = await client.GetAsync("/Admin/Grants");
    ...
    {
        IGrantListService service = MockService(EmptyResult());
        HttpClient client = CreateClient(service);
    ...
    public async Task GetIndex_RendersGrantFiltersAndPreservesThemInNavigationAndRevocation()
    {
        ...
        HttpResponseMessage response =
            await client.GetAsync(
                "/Admin/Grants?SubjectId=user-101&ClientId=client-1&TypeFilter=refresh_token&PageNumber=2");
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        string? previousLink = document.QuerySelector("a[data-pagination=previous]")?.GetAttribute("href");
        ...
        Assert.Equal("user-101",
            document.QuerySelector("#revoke-grant-modal input[name=SubjectId]")?.GetAttribute("value"));
        Assert.Equal("client-1",
            document.QuerySelector("#revoke-grant-modal input[name=ClientId]")?.GetAttribute("value"));
        Assert.Equal("refresh_token",
            document.QuerySelector("#revoke-grant-modal input[name=TypeFilter]")?.GetAttribute("value"));
    }
}